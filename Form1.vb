...
Public Class Form1
    ...
    Dim _pnlLeft_Index As Integer = -1
    ...
    'Dim myFavoriteList As New Collections.Hashtable
            ...
            For Each y As Xml.XmlNode In x.ChildNodes
                ...
                Else
                    ...
                    myGamesMenu.Add(strTag, New NewUIListItem() With {.Dock = DockStyle.Left, .Height = 48, .Width = pnlLeft.Width - 28,
                                 ...
                                 .ClickHandler = Sub()
                                                     If y.Attributes("romMethod") IsNot Nothing Then
                                                         ListRom(y.Attributes("romType").Value,
                                                             strTag,
                                                             y.Attributes("romPath").Value,
                                                             My.Resources.ResourceManager.GetObject(y.Attributes("image").Value), y.Attributes("romMethod").Value)
                                                     Else
                                                         ListRom(y.Attributes("romType").Value,
                                                             strTag,
                                                             y.Attributes("romPath").Value,
                                                             My.Resources.ResourceManager.GetObject(y.Attributes("image").Value))
                                                     End If
                                                 End Sub,
                                 .AltClickHandler = Sub()
                                                        If y.Attributes("romMethod") IsNot Nothing Then
                                                            ListRom(y.Attributes("romType").Value,
                                                                ...
                                                                My.Resources.ResourceManager.GetObject(y.Attributes("image").Value), y.Attributes("romMethod").Value, Function(r As IRom) As Boolean
                                                                                                                                                                          Return IsFavorite(y.Attributes("romType").Value, r.Name)
                                                                                                                                                                      End Function)
                                                        Else
                                                            ListRom(y.Attributes("romType").Value,
                                                                strTag,
                                                                y.Attributes("romPath").Value,
                                                                My.Resources.ResourceManager.GetObject(y.Attributes("image").Value),, Function(r As IRom) As Boolean
                                                                                                                                          Return IsFavorite(y.Attributes("name").Value, r.Name)
                                                                                                                                      End Function)
                                                        End If
                                                    End Sub
                            })
...
#Region "ROM Acquistion"
    ...
    Private Function ListMAMEDefaultClause(additional As Func(Of XElement, Boolean)) As Func(Of XElement, Boolean)
        Dim bads = GetMarkedAsBad("MAME")
        Return (Function(element As XElement) As Boolean
                    If bads.Contains(element.@<name>) Then Return False
    ...
    Private Sub ListMAME(whereClause As Func(Of XElement, Boolean))
        ...
        For Each i In GetFavorites("MAME")
...
#End Region
    Private Function NewSubMenu() As FlowLayoutPanelEx
        ...
        If subMenus Is Nothing Then
            ReDim subMenus(0)
            ReDim _pnlLeft2_Index(0)
        Else
            ReDim Preserve subMenus(UBound(subMenus) + 1)
            ReDim Preserve _pnlLeft2_Index(UBound(_pnlLeft2_Index) + 1)
        End If
        ...
        With DirectCast(subMenus(UBound(subMenus)), FlowLayoutPanelEx)
            ...
            AddHandler .VisibleChanged, AddressOf pnlLeft2_VisibleChanged
            .Visible = False
            .WrapContents = False
            ...
            .BringToFront()
            '.Visible = True
            pnlLeft2_Index(UBound(_pnlLeft2_Index)) = 0
            '.Update()
    ...
    Private Sub CloseSubMenu()
        If subMenus IsNot Nothing Then
            ...
            If SubMenuDepth = -1 Then
                SubMenuDepth = 0
                Erase subMenus
                Erase _pnlLeft2_Index
            Else
                With DirectCast(subMenus(SubMenuDepth), FlowLayoutPanelEx)
                    .Visible = False
                    pnlLeft.Controls.Remove(subMenus(SubMenuDepth))
                    RemoveHandler .VisibleChanged, AddressOf pnlLeft2_VisibleChanged
                    '.Dispose()
                    'debugobject = Nothing
                    If SubMenuDepth > 0 Then pnlLeft.Width -= 70
                End With
                ReDim Preserve subMenus(UBound(subMenus) - 1)
                ReDim Preserve _pnlLeft2_Index(UBound(_pnlLeft2_Index) - 1)
            End If
        ...
        If subMenus IsNot Nothing Then
            ...
            Else
                For z = SubMenuDepth To 1 Step -1
                    RemoveHandler subMenus(z).VisibleChanged, AddressOf pnlLeft2_VisibleChanged
                ...
                With DirectCast(subMenus(0), FlowLayoutPanelEx)
                    ...
                    '.Dispose()
                    'debugobject = Nothing
                    If SubMenuDepth > 0 Then pnlLeft.Width -= 70 * (SubMenuDepth)
                End With
    ...
    Private Sub ProcessSubMenuKeyDown(sender As Object, e As KeyEventArgs)
        Select Case e.KeyCode
            Case myKeybindings("Action").KeyboardKey
                If pnlLeft2_Index(SubMenuDepth - 1) >= 0 AndAlso subMenus(SubMenuDepth - 1).Controls.Count Then DirectCast(subMenus(SubMenuDepth - 1).Controls(pnlLeft2_Index(SubMenuDepth - 1)), NewUIListItem).PerformClick()
                e.Handled = True
                'End Select
                'SubMenuDepth += 1
            ...
            Case myKeybindings("Down").KeyboardKey  'Keys.Down
                pnlLeft2_Index(SubMenuDepth - 1) += 1
                e.Handled = True
    ...
    Private Sub ProcessLeftMenuKeyDown(sender As Object, e As KeyEventArgs)
        Select Case e.KeyCode
            Case myKeybindings("Action").KeyboardKey

                Select Case pnlLeft_Current
                                'games
                    Case "pnlGames"
                        With NewSubMenu()
                            ...
                            myList.AppsMenu = New List(Of NewUIListItem)({New NewUIListItem("Favorite", My.Resources.favorite, Sub()
                                                                                                                                   If IsFavorite(myList.Tag, myList.SelectedItem.Name) Then
                                                                                                                                       myList.SelectedItem.Favorite = False
                                                                                                                                       myList.RefreshIndex(myList.Index)
                                                                                                                                       RemoveFromFavorites(myList.Tag, myList.SelectedItem.Name)
                                                                                                                                   Else
                                                                                                                                       myList.SelectedItem.Favorite = True
                                                                                                                                       myList.RefreshIndex(myList.Index)
                                                                                                                                       AddToFavorites(myList.Tag, myList.SelectedItem.Name)
                                                                                                                                   End If
                                                                                                                               End Sub),
                                                                                      New NewUIListItem("Mark as Bad", My.Resources.notifications_powermenu_icon_close, Sub()
                                                                                                                                                                            If IsMarkedAsBad(myList.Tag, myList.SelectedItem.Name) Then
                                                                                                                                                                                MarkAsNotBad(myList.Tag, myList.SelectedItem.Name)
                                                                                                                                                                            Else
                                                                                                                                                                                MarkAsBad(myList.Tag, myList.SelectedItem.Name)
                                                                                                                                                                            End If
                                                                                                                                                                        End Sub),
                    ...
                    Case "pnlPower"
                        With NewSubMenu()
                            .Controls.Add(New NewUIListItem() With {.Text = "Close Menulator", .Image = My.Resources.notifications_powermenu_icon_close, .Dock = DockStyle.Left, .Height = 48, .Width = .Width, .ClickHandler = Sub()
                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                    'Application.Restart()
                                                                                                                                                                                                                                End Sub})
                            .Controls.Add(New NewUIListItem() With {.Text = "Restart Menulator", .Image = My.Resources.notifications_powermenu_icon_restart, .Dock = DockStyle.Left, .Height = 48, .Width = .Width, .ClickHandler = Sub()
                                                                                                                                                                                                                                        'Me.Close()
                                                                                                                                                                                                                                        Application.Restart()
                                                                                                                                                                                                                                    End Sub})
                            ...
                            .Controls.Add(New NewUIListItem() With {.Text = "Restart", .Image = My.Resources.notifications_powermenu_icon_restart, .Dock = DockStyle.Left, .Height = 48, .Width = .Width, .ClickHandler = Sub()
                                                                                                                                                                                                                              Process.Start("shutdown", "-r -f -t 00")
                                                                                                                                                                                                                          End Sub})
                            .Controls.Add(New NewUIListItem() With {.Text = "Shutdown", .Image = My.Resources.notifications_powermenu_icon_shutdown, .Dock = DockStyle.Left, .Height = 48, .Width = .Width, .ClickHandler = Sub()
    ...
    Private Sub me_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown ', myList.KeyDown
        ...
        ElseIf pnlLeft.Visible Then
            ProcessLeftMenuKeyDown(sender, e)
            If e.Handled Then
                e.SuppressKeyPress = True
                Exit Sub
            End If
        ...
        Select Case e.KeyCode
            ...
            Case myKeybindings("AltAction").KeyboardKey
                If pnlLeft.Visible AndAlso (SubMenuDepth > 0 And subMenus(0).Controls(pnlLeft2_Index(0)).Tag = "MAME") Then
                    With NewSubMenu()
                        .Controls.AddRange(New NewUIListItem() {
                                                                New NewUIListItem("Display Favorites", My.Resources.favorite, Sub()
                                                                                                                                  ...
                                                                                                                                  If pnlLeft2_Index(SubMenuDepth - 1) >= 0 AndAlso subMenus(SubMenuDepth - 1).Controls.Count Then favs = GetFavorites(DirectCast(subMenus(0).Controls(pnlLeft2_Index(0)), NewUIListItem).Tag)
                                                                                                                                  If DirectCast(subMenus(0).Controls(pnlLeft2_Index(0)), NewUIListItem).Tag = "MAME" Then
                                                                                                                                      ListMAME(ListMAMEDefaultClause(Function(element As XElement) As Boolean
                                                                                                                                                                     ...
                                                                                                                                                                     End Function))
                                                                                                                                  Else
                                                                                                                                      myGamesMenu(subMenus(SubMenuDepth - 1).Controls(pnlLeft2_Index(SubMenuDepth - 1)).Tag).PerformAltClick()
                                                                                                                                  End If
            ...
            Case myKeybindings("Cancel").KeyboardKey
                If pnlLeft_Current = "pnlFiles" Then
                    Try
...
#End Region
    ...
    Private Property pnlLeft_Index As Integer
        Get
            Return _pnlLeft_Index
        End Get
        Set(value As Integer)
            If value < 0 Then value = 0
            If value > FlowLayoutPanel1.Controls.Count - 1 Then value = FlowLayoutPanel1.Controls.Count - 1
            If _pnlLeft_Index >= 0 Then FlowLayoutPanel1.Controls(_pnlLeft_Index).BackColor = FlowLayoutPanel1.BackColor
            _pnlLeft_Index = value
            FlowLayoutPanel1.Controls(_pnlLeft_Index).BackColor = SystemColors.ActiveBorder : FlowLayoutPanel1.ScrollControlIntoView(FlowLayoutPanel1.Controls(_pnlLeft_Index))
        End Set
    End Property
    Private ReadOnly Property pnlLeft_Current As String
        Get
            Return FlowLayoutPanel1.Controls(_pnlLeft_Index).Name
        End Get

    End Property
    Dim _pnlLeft2_Index() As Integer
    Private Property pnlLeft2_Index(index As Integer) As Integer
        Get
            Return _pnlLeft2_Index(index)
        End Get
        Set(value As Integer)
            If index > UBound(subMenus) Then Return
            If value < 0 Then value = 0
            If value > subMenus(index).Controls.Count - 1 Then value = subMenus(index).Controls.Count - 1
            If _pnlLeft2_Index(index) >= 0 AndAlso _pnlLeft2_Index(index) <= subMenus(index).Controls.Count - 1 Then subMenus(index).Controls(_pnlLeft2_Index(index)).BackColor = subMenus(index).BackColor
            _pnlLeft2_Index(index) = value
            If _pnlLeft2_Index(index) >= 0 AndAlso _pnlLeft2_Index(index) <= subMenus(index).Controls.Count - 1 Then subMenus(index).Controls(_pnlLeft2_Index(index)).BackColor = SystemColors.ActiveBorder : subMenus(index).ScrollControlIntoView(subMenus(index).Controls(_pnlLeft2_Index(index)))

        End Set
    End Property
    Private Sub pnlLeft2_VisibleChanged(sender As Object, e As EventArgs) 'Handles debugobject.VisibleChanged
        If SubMenuDepth > 0 Then
